...
import { sendCoachMessageAction, type SendMessageFormState } from "../actions";

const initialState: SendMessageFormState = { status: "idle" };

export function MessageForm({ engagementId }: { engagementId: string }) {
  const [state, formAction, isPending] = useActionState(sendCoachMessageAction, initialState);
